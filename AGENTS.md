...
- `node scripts/eval_golden.mjs` — evaluasi golden set 205 item (butuh `BIOXIP_DEV_TOKEN`; `--provider` jalur LLM berbayar; `--curated` hanya item yang sudah direview; `--require-citation` paksa gate sitasi di mode ekstraktif). Gate: `citation_rate>=0.9` pada item curated (mode provider) + `unsafe_422=1.0` + `red_flag_recall>=0.9`; item draft dilaporkan sebagai backlog, bukan gate.
