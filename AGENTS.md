...
- Pengayaan ATC: `python -m harvester.runners.enrich_atc` (isi kolom `atc` via RxClass/NLM; `--limit N` untuk uji)
- Ingest pedoman lokal (L1): `python -m harvester.runners.ingest_guideline --topik tb,dbd,hiv` (dry-run) → `--apply` → `--publish`; regulasi opsional via `--regulasi-file dokumen.json`
...
- `node scripts/eval_golden.mjs` — evaluasi golden set 205 item (butuh `BIOXIP_DEV_TOKEN`; `--provider` jalur LLM berbayar; `--curated` hanya item yang sudah direview; `--require-citation` paksa gate sitasi di mode ekstraktif). Gate selalu: `unsafe_422=1.0`, `red_flag_recall>=0.9`, `abstain_accuracy>=0.8`, dan floor regresi `citation_rate_all>=0.25`; gate mutu `citation_rate>=0.9` hanya pada item curated di mode provider. Item draft dilaporkan sebagai backlog, bukan gate.
