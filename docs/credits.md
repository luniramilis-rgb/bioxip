...
1. **Sign in wajib** untuk menggunakan bioXip (search & data) — mulai dari versi gratis, demi data pengguna. Metode: **Google OAuth** + **magic link email / OTP 6 digit**. (Apple Sign In **tidak dipakai** — keputusan 2026-09-10.)
...
## 14. Alur auth (Google OAuth + magic link/OTP)
```
1. Pengunjung menekan "Masuk" → pilih "Lanjutkan dengan Google" atau "Kirim tautan/kode email".
2. Supabase Auth menyelesaikan OAuth, atau mengirim magic link + OTP 6 digit.
...
```
Guardrail: rate limit pengiriman magic link/OTP per email & per IP; verifikasi email; aktifkan *identity linking* (email sama via Google & magic link tidak menjadi dua akun).
⚠️ **In-app browser** (Instagram/Facebook/TikTok/Line) memblokir OAuth Google → deteksi UA & tampilkan layar "Buka di Chrome/Safari" (`docs/principles.md` §5).
⚠️ **Keterkiriman email** wajib dijaga: domain pengirim sendiri + SPF/DKIM (mis. `no-reply@bioxip.id`), uji ke Gmail, dan sediakan OTP sebagai jalur cadangan bila tautan magic link gagal dibuka.
