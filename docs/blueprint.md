...
- 2026-09-10 — Keputusan monetisasi (lihat `docs/credits.md`): top-up via **Xendit**; **DeepSeek** sebagai provider AI tunggal (dengan abstraksi provider); **percakapan disimpan untuk evaluasi** (retensi 90 hari, boleh dihapus, tanpa data pasien); pembeda gratis vs berbayar = **mode eksplisit** (segmented: `Cari bukti · gratis` vs `Tanya AI · kredit`), search-first, estimasi kredit tampil sebelum eksekusi, endpoint search/data tidak pernah menyentuh ledger; tier free mendapat kuota AI harian kecil (2.000 kredit ≈ 3 jawaban/hari).
