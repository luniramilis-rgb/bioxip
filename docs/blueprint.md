...
- 2026-09-10 — **Prinsip produk mengikat**: `docs/principles.md`. Pasar Indonesia; **mobile-first** (95%+, PWA, target 360px, LCP <2,5 dtk); **sign in Google + Apple + magic link** (catatan: Apple "Hide My Email" & OAuth gagal di in-app browser wajib ditangani); **share-first** via WhatsApp/Instagram/Threads/X dengan Web Share API + `share_target` PWA; **halaman share publik** (topik, drug card ringkas, snapshot jawaban) meski gate login aktif; loop rujukan `?ref=` + K-factor; pembayaran lokal; checklist rilis mobile wajib lulus.
