...
- 2026-09-10 — Model akses FINAL: **tanpa trial**. Search & data gratis selamanya (tanpa login); **AI terkunci bila saldo Rp0**. Saldo dibeli bertingkat (Rp50rb/100rb/150rb/500rb), dalam **Rp**, **tanpa kedaluwarsa**, **tanpa bonus**, markup **12×** biaya asli DeepSeek V4.1 Flash (cache hit $0,006 / miss $0,30 / output $1,20 per 1 juta token). Top-up Xendit (QRIS + VA + e-wallet). Optimasi **prompt cache** (prefix stabil) jadi pengungkit margin utama.
...
- 2026-09-10 — Rencana implementasi bertahap ada di `docs/plan.md`: Sprint 1 PubMed E-utilities (gratis) → Sprint 2 Auth+Ledger → Sprint 3 Grounded internal + golden 30 → Sprint 4 AI proxy + debit DeepSeek → Sprint 5 UI gratis vs AI → Sprint 6 Xendit → Sprint 7 kualitas retrieval & data. Review apoteker/dokter dan lapisan lokal berjalan paralel.
