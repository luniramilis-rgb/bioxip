...
**Status (2026-09-11):** Fase 0–2 selesai; **gate review manusia DIHAPUS** (keputusan produk) → publikasi dipandu validasi otomatis + provenance (migrasi `020`, runner `--validate` internal saat `--publish`, kolom `source_tier`/`retrieved_at`). Sumber resmi penuh dari **API e-Fornas** (`/api/daftar-obat`, 1.254 baris → 663 obat unik) diimpor via `--from-fornas-api` dan **dipublikasikan otomatis**. Prinsip aman-tervalidasi tetap: migrasi aditif, validasi `validate_records` sebelum publish, provenance/`source_tier`, versioning `dataset_version`, fallback `_drugs.json`, dry-run. **Fase 3 selesai**: `/api/drug` membaca Postgres (view publik) via `functions/_formulary.js` dengan **feature flag `FORMULARY_DB` (default OFF)** + cache edge + fallback JSON; `/api/suggest` memakai DB bila flag ON; unit test `tests/formulary_unit.js` (+CI). **Fase 5 selesai**: adapter literatur `functions/_literature/{crossref,doaj,linkout}.js` (opt-in env `LIT_SOURCES`, default OFF) + `neliti_oai.js` (harvest metadata, bukan live search); `/api/search` menambah sumber + link-out OneSearch (Garuda via `GARUDA_SEARCH_URL`), cache `search:v8`; `tests/literature_unit.js` + smoke. **Fase 6 selesai**: validator statis `scripts/validate_formulary.js` (migrasi 017–022, gate review dihapus, hardening pencarian, adapter literatur, flag OFF, Garuda tidak di-hardcode) + live `scripts/validate_formulary_live.mjs` (anon baca view & RPC, tolak tabel dasar, hardening wildcard, coverage); statis masuk CI `validate.yml`. **Fase 7 selesai**: verifikasi produksi (`validate_security_live`/`validate_formulary_live`/`validate_api` ALL PASS; migrasi 017–022 sinkron); **ATC** diisi via RxClass (`harvester/runners/enrich_atc.py`, 421/652 ≈65%); flag produksi aktif `FORMULARY_DB=on` + `LIT_SOURCES=crossref,doaj`. `/api/drug` mengutamakan katalog JSON (ATC + catatan keselamatan) dan memakai DB untuk obat di luar katalog. Backlog prarilis: ATC untuk sisa obat, renderer UI `doc_type:"link"` + URL Garuda terverifikasi (untuk mengaktifkan `linkout`), dan konfirmasi izin/atribusi API Kemenkes.
---
