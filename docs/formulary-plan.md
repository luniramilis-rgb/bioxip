...
**Status (2026-09-11):** Fase 0–2 selesai; **gate review manusia DIHAPUS** (keputusan produk) → publikasi dipandu validasi otomatis + provenance (migrasi `020`, runner `--validate` internal saat `--publish`, kolom `source_tier`/`retrieved_at`). Sumber resmi penuh dari **API e-Fornas** (`/api/daftar-obat`, 1.254 baris → 663 obat unik) diimpor via `--from-fornas-api` dan **dipublikasikan otomatis**. Prinsip aman-tervalidasi tetap: migrasi aditif, validasi `validate_records` sebelum publish, provenance/`source_tier`, versioning `dataset_version`, fallback `_drugs.json`, dry-run. **Fase 3 selesai**: `/api/drug` membaca Postgres (view publik) via `functions/_formulary.js` dengan **feature flag `FORMULARY_DB` (default OFF)** + cache edge + fallback JSON; `/api/suggest` memakai DB bila flag ON; unit test `tests/formulary_unit.js` (+CI). **Fase 4 selesai** (migrasi `021_drug_search.sql`, bukan `018` karena nomor sudah terpakai): view publik menyertakan `search_tsv`; RPC `fn_drug_search(p_query,p_limit)` (SQL STABLE, baca-saja, grant anon/authenticated) memberi peringkat exact nama/INN > prefix > FTS > trigram; `_formulary.searchFormularyDrugs` memakai RPC (suggest mengutamakan RPC, fallback ILIKE). Fase 5+ (adapter literatur) belum dikerjakan. Catatan: ATC belum tersedia dari API Fornas (perlu sumber terpisah).
---
