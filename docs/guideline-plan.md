...
**Status (2026-09-12):** **L0 selesai**; **L1 selesai**; **L2 selesai** — `functions/_guideline.js` (`guidelineEnabled` `GUIDELINE_DB`, `detectGuidelineTopic`, `searchGuidelines` via RPC + cache) diintegrasikan ke `/api/search` (pedoman **diprioritaskan di depan** untuk pertanyaan terkait; `source_tier=official`; badge tier/edisi/locator di UI), cache `search:v9`; unit test `tests/guideline_unit.js` + smoke (flag OFF/ON) masuk CI. **L3 selesai** — pedoman lokal dialirkan sebagai **evidence terdepan di jawaban AI** (`api/ai/chat.js`: `searchGuidelines` → `toEvidence`, renumber; meta badge tier/edisi/locator diteruskan ke SSE `citation` dan ditampilkan di panel AI) + **link-out BPOM (CekBPOM)** ditambahkan ke `linkoutEntries`. **L4–L5 belum** (pedoman profesi, buku link-out). Kebijakan fetch-first; gate lisensi/MoU dihapus.
