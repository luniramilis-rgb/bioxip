...
**Progres (2026-09-12):** **N0–N5 selesai.** N1: `minInterval` diekspor + `tests/pubmed_unit.js`; **`NCBI_API_KEY` (36 char) + `NCBI_EMAIL` diset di produksi** (Cloudflare Pages) dan redeploy → throttle 100 ms (10 req/dtk). Bukti: 3 kueri `/api/search` (TB, dengue, hipertensi) → `pubmed=10`, `notes=0` (sebelumnya sempat `notes=1` karena throttle). N2: patterns.js 5 pola + chips + precache v6. N3: chip Bukti klinis + ekspor Vancouver/APA + bullet klaim kunci. N4: reranking local-first via `source_tier`. N5: 5 topik konten-saja (`golden:false`) + indikator "Kekuatan bukti". Catatan: golden set tetap 205 (curated 40/draft 165).
